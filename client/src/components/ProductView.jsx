...
export default function ProductView({ products }) {
  ...
  const handleCancelAddSize = () => {
    ...
    const relatedProducts=products.filter(prod=>prod.category1.value===product.category1.value && prod.category2.value===product.category2.value && prod._id!==product._id)

  
  return (
    <div className="mx-0">
      <div
        className={`zzz flex items-center top-0 left-0 h-full w-full mt-10 `}
      >
        <div className=" relative lg:h-[60%] w-[100%] xs:w-[95%] md:w-[90%] lg:w-[80%] mx-auto bg-white py-8 lg:py-16 px-4 lg:px-8 lg:p-16 ">
          <div className="flex flex-col xs:flex-row flex-wrap w-full h-full ">
            ...
            <div className="w-full xxs:w-11/12 mx-auto xs:w-1/2  text-lightGray flex flex-col pl-6 lg:pl-12 pt-10 xs:pt-0 items-start gap-4 overflow-y-auto">
              <div className=" w-full inline-flex justify-between items-center">
                <h5 className=" text-sm md:text-base lg:text-xl">{product.category2.value || ""}, {product.category1.value || ""}</h5>
                {user && 
                <div className="flex flex-row flex-nowrap items-center justify-center">
                  <FaEdit className="size-12 hover:text-dark hover:cursor-pointer p-3" onClick={()=>navigate(`/update-product/${product._id}`)} /> 
                  <MdDelete className="size-12 hover:text-dark hover:cursor-pointer p-3" onClick={()=>handleDeleteProduct(product._id)} />
                  </div>
                 }
              </div>
              <h2 className=" text-black text-xl md:text-2xl lg:text-3xl">{product.name}</h2>
              ...
              {user && <button className=" hover:text-dark text-xl font-bold" onClick={()=>setShowAddSizeForm(true)}>+ size</button>}
              </div>
                  ...
                  (size, index) =>
                    size.qty > 0 && (
                      <span
                        ...
                        className={` ${ size.size == selectedSize ? "bg-black" : "bg-lighterGreen"} text-white text-sm lg:text-base rounded-md p-1 hover:bg-black hover:cursor-pointer`}
                      >
                        {size.size}
                      </span>
                    )
          ...
          {relatedProducts &&relatedProducts.length > 0 && <div className="w-full py-20 ">
          <h1 className="pb-10">Related Products</h1>
          <div className="grid grid-cols-2 md:grid-cols-3 2xl:grid-cols-4 w-full  " >
            {relatedProducts && relatedProducts.map((product) => (
              <div key={product._id} className="mx-auto w-32 xxs:w-40 xs:w-56  sm:w-64 pb-10  relative hover:cursor-pointer " onClick={()=>navigate(`/product/${product._id}`)}>
                  {product.priceBeforeDisc !== 0 && <span className=" absolute text-center text-sm xs:text-base m-auto px-1   pt-[10px] xxs:pt-[11px] h-10 xxs:h-11  w-10 xxs:w-11 -top-3 -right-3 bg-lightGreen text-white rounded-full z-50">Sale!</span>}
                <div className="w-full pb-4   z-0 overflow-hidden" >
                  <img src={product.img} alt="shoesImg" className="w-full hover:scale-125 duration-300 ease-linear z-10"  />
                  </div>
                <h4 className=" text-xs xxs:text-sm xs:text-base sm:text-lg">{product.name}</h4>
                <h4 className=" text-sm xxs:text-base xs:text-lg sm:text-xl">{product.priceBeforeDisc ? <span className="pr-2 xs:pr-5 line-through text-lighterGray">${(product.priceBeforeDisc).toFixed(2)}</span> : ""}<span className="text-lightGray">${(product.price.toFixed(2))}</span></h4>
              </div>
            ))}
          </div>
          </div>}
